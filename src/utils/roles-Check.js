...
const rolesCheck = (req, res, next) => {
    ...
    try {
        ...
        const sql = 'select permision from roles where userId = ?';

        req.mysql.query(sql, id, (error, results) => {
            if(error){
                console.log("roles check Error : ", error.message);
                return res.status(500).json({ message : "데이터 전송에 오류가 발생하였습니다." });
            } else if(results.length === 0){
                return res.status(404).json({ message : "유저가 존재하지 않습니다." });
            } else if(results.length !== 0 && results[0].permision === 'user'){
                return res.status(403).json({ message : "어드민 권한이 존재하지 않습니다." });
            } else if(results.length !== 0 && results[0].permision === 'admin'){
                next();
            }
        });
